...
class Graph{
	...
	boolean bfsUtil( boolean[] visited, int vertex ) {
		Queue<Node> queue = new LinkedList<Node>();
		// let's assume that the parent of the root node is -1
		queue.add( new Node(vertex,-1) );
		visited[vertex] = true;
		// run the loop until the size of queue becomes zero
		while( queue.isEmpty() != true ) {
			Node element = queue.remove();
			for( int i=0;i<this.vertices;i++ ) {
				if( this.adj_matrix[element.vertex][i] ) {
					if( visited[i] ) {
						if( i != element.parent ) return true;
					} else {
						queue.add( new Node(i,element.vertex) );
						visited[i] = true;
					}
				}
			}
